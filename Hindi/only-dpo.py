#!/usr/bin/env python3
"""
Hindi-only Private Training: DPO only with DP vs PDP
...
- Reports accuracy on Hindi test after DPO
...
SIGMA = 4.0                       # Noise multiplier for DP
LR = 1e-4
EPOCHS = 3
...
def tokenize_dpo(pairs, tokenizer, max_len=512):
    ...
    data = {"c_ids": [], "c_mask": [], "c_lab": [], "r_ids": [], "r_mask": [], "r_lab": [], "correct_idx": []}
    for p in pairs:
        def enc(text):
        ...
        data["c_ids"].append(c_ids)
        data["c_mask"].append(c_mask)
        data["c_lab"].append(c_lab)
        data["r_ids"].append(r_ids)
        data["r_mask"].append(r_mask)
        data["r_lab"].append(r_lab)
        data["correct_idx"].append(torch.tensor(p["correct_idx"]))
    ...
    tensors = [
        torch.stack([pad(t, max_c if k.startswith("c_") else max_r) for t in data[k]])
        if k != "correct_idx" else torch.stack(data[k])
...
def main():
    ...
    log_filename = f"logs/hindi_private_dp_vs_pdp_onlydpo_pub{PUBLIC_SUBSET_SIZE}_bs{BATCH_SIZE}_sigma{SIGMA}_k{SUBSPACE_K}_lr{LR}_epochs{EPOCHS}_seed{SEED}.log"
    ...
    # ==================== DP PATH (DPO only) ====================
    logger.info("\n" + "="*80)
    logger.info("STARTING DP PATH: DPO-DP (no SFT)")
    ...
    # ==================== PDP PATH (DPO only) ====================
    logger.info("\n" + "="*80)
    logger.info(f"STARTING PDP PATH ({NUM_FOLDS} folds, public subset size={PUBLIC_SUBSET_SIZE}) - DPO only")
    pdp_dpo_accs = []
    ...
    # ==================== FINAL SUMMARY ====================
    logger.info("\n" + "="*80)
    logger.info("FINAL RESULTS SUMMARY (Hindi Test Accuracy) - DPO ONLY")
    logger.info("="*80)
